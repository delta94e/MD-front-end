...
> 🎙️ *"The key insight is that index[i] tells us where arr[i] should go. With an auxiliary array, it's trivially result[index[i]] = arr[i]. For O(1) space, I recognize that any permutation decomposes into cycles. I follow each cycle, swapping elements into their correct positions. Each element is swapped at most once, giving O(n) total."*

### Think Out Loud — Quá trình suy nghĩ

```
  🧠 BƯỚC 1: Đọc đề → phát hiện keywords
    "reorder" + "given indexes" → permutation!
    index[i] = vị trí ĐÍCH cho arr[i]
    → result[index[i]] = arr[i]!

  🧠 BƯỚC 2: Solution đơn giản
    "Mảng phụ result[] → đặt từng phần tử → done!"
    → O(n) time, O(n) space
    → 3 dòng code chính!

  🧠 BƯỚC 3: Follow-up: "In-place?"
    "Permutation = cycles → swap theo cycle!"
    "while index[i] ≠ i: swap cả arr và index"
    → O(n) time, O(1) space

  🧠 BƯỚC 4: Edge cases
    → 1 phần tử: trivial
    → Identity permutation: không swap nào
    → index[] đã modify sau in-place approach

  🎙️ Interview phrasing:
    "I'll start with the straightforward O(n) space approach:
     create a result array and place each element at its target
     index. For the follow-up, I can do it in-place by following
     the cycle structure of the permutation — swapping elements
     along each cycle until everyone is in the right seat."
```
...
```
  ...
  1. REVERSE mapping: "Tại vị trí i, lấy phần tử TỪ index[i]"
     → result[i] = arr[index[i]]   ← KHÁC BÀI!
     → Đọc kỹ đề: "moves TO" vs "comes FROM"!

  2. Reorder theo sort order
     → Sort mảng → index = vị trí mới sau sort
     → Áp dụng cùng technique!

  3. Inverse permutation
     → Cho index[], tìm inverse sao cho index[inv[i]] = i
     → Ứng dụng: "undo" 1 reorder!

  4. Apply permutation K lần
     → Permutation exponentiation → O(n log k)
     → Advanced: cycle decomposition + modulo!

  5. Sort 2 mảng theo 1 key
     → Sort index → reorder cả arr theo index đã sort
     → Dùng zip + sort!
```
...
```
  ...
  │  ────────────────────────────────────────────────        │
  │  Reorder by Index ⭐   Mảng phụ/Cyclic     O(n)        │
  │  Cyclic Sort           while swap           O(n)        │
  │  Find Missing Number   Cyclic Sort variant  O(n)        │
  │  First Missing Positive Cyclic Sort         O(n)        │
  │  Shuffle Array         Fisher-Yates         O(n)        │
  └──────────────────────────────────────────────────────────┘

  KEY INSIGHT:
  → Có index[] chỉ vị trí → result[index[i]] = arr[i]
  → In-place → Cyclic Sort (swap theo cycle)
  → Mọi permutation = tập hợp cycles!
```
